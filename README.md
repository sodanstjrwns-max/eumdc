...
## 📚 2026-07-21 9차 증량: 용어사전 15개 추가 (누적 130개 완료)
- **대상(9차)**: 직접수복/브래킷/치조골보존술/관절원판/개구/치아삭제/교합간섭/치은/이신경/본딩/구강상악동누공/치관/아쿠아케어/치태조절/소구치 — 보존·교정·구강외과·해부 계열 롱테일 키워드 (기존 600자대 단문 항목 위주로 선정)
- **본문+FAQ 합산 3,000자+**: 각 용어 h3 섹션 8~13개 + 맞춤 FAQ 5개(가시 FAQ ↔ FAQPage JSON-LD 1:1), 전 15개 3002~3131자 검증 완료 (실서비스 `/dictionary/tooth-crown`, `/dictionary/bracket`, `/dictionary/articular-disc`, `/dictionary/premolar` 렌더링 확인: h3=12~13, FAQ=5, JSON-LD Question=5)
- **마이그레이션**: 0153(본문 1배치) + 0154~0158(탑업 5개, tooth-crown 미달분 추가 보강 포함) — 로컬/원격(production) D1 적용 완료
- **잔여**: ~89개 용어 미증량 (620~800자대) — 다음 배치 후보

## 📚 2026-07-21 8차 증량: 용어사전 15개 추가 (누적 115개 완료)
- **대상(8차)**: 오스템/GBR/심미보철/교합/근관/치수/법랑질/상아질/아말감/GIC/치수복조술/MTA/거타퍼차/치근단절제술/치조골 — 기초 해부·보존·근관 계열 롱테일 키워드
- **본문+FAQ 합산 3,000자+**: 각 용어 h3 섹션 9~11개 + 맞춤 FAQ 5개(가시 FAQ ↔ FAQPage JSON-LD 1:1), 전 15개 3011~3179자 검증 완료 (실서비스 `/dictionary/osstem` 렌더링 확인: h3=11, FAQ=5)
- **마이그레이션**: 0145(본문 1배치) + 0146~0152(탑업 7개) — 로컬/원격 D1 적용 완료
- **잔여**: ~104개 용어 미증량 (620~790자대) — 다음 배치 후보

## 📚 2026-07-21 7차 증량: 용어사전 15개 추가 (누적 100개 완료 🎉)
- **대상(7차)**: 픽스처/지대주/힐링어버트먼트/포스트코어/PFM/올세라믹/티타늄/러버댐/치과현미경/디지털인상/구강스캐너/캐드캠/디지털가이드/왁스업/쉐이드매칭 — 임플란트 부품·재료·디지털 장비 계열 롱테일 키워드
- **본문+FAQ 합산 3,000자+**: 각 용어 h3 섹션 7~8개 + 맞춤 FAQ 5개(가시 FAQ ↔ FAQPage JSON-LD 1:1), 전 15개 3003~3222자 검증 완료
- **마이그레이션**: 0140~0142(본문 3배치) + 0143~0144(탑업 2개) — 로컬/원격 D1 적용, 실서비스 렌더링 검증 완료 (h3=13, JSON-LD FAQ=5)
- **잔여**: ~119개 용어 미증량 (535~790자대) — 다음 배치 후보 / 지역 랜딩 증량 대기

## 📚 2026-07-21 6차 증량: 용어사전 15개 추가 (누적 85개 완료)
- **대상(6차)**: 농양/구순포진/설태/타액/턱골절/턱관절잠김/구강위생/치간칫솔/임시보철/치아성형/심미라인/치관길이연장술/잇몸절제술/플랩수술/치주낭 — 응급·치주·심미·예방 계열 롱테일 키워드
- **본문+FAQ 합산 3,000자+**: 각 용어 h3 섹션 6~8개 + 맞춤 FAQ 5개(가시 FAQ ↔ FAQPage JSON-LD 1:1), 전 15개 3008~3228자 검증 완료
- **마이그레이션**: 0133~0135(본문 3배치) + 0136~0139(탑업 4개) — 로컬/원격 D1 적용, 실서비스 렌더링 검증 완료
- **잔여**: ~134개 용어 미증량 (535~790자대) — 다음 배치 후보 / 지역 랜딩 증량 대기

## 📚 2026-07-21 5차 증량: 용어사전 15개 추가 (누적 70개 완료)
- **대상(5차)**: 구강암/치아외상/2차충치/임시치아/즉시임플란트/미니임플란트/네비게이션임플란트/전악임플란트/재근관치료/치태/파노라마/소대절제술/과잉치/유치/치근활택술 — 응급·진단·임플란트 계열 롱테일 키워드
- **본문+FAQ 합산 3,000자+**: 각 용어 h3 섹션 6~8개 + 맞춤 FAQ 5개(가시 FAQ ↔ FAQPage JSON-LD 1:1), 전 15개 3017~3254자 검증 완료
- **마이그레이션**: 0127~0129(본문 3배치) + 0130~0132(탑업) — 로컬/원격 D1 적용, 실서비스 렌더링 검증 완료
- **잔여**: ~149개 용어 미증량 (546~790자대) — 다음 배치 후보 / 지역 랜딩 증량 대기
