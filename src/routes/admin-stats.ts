...
import { verifyHmacToken, getAuthSecret } from './auth'
...
// ────────────────────────────────────────────────────────────
// 라우트 — 기존 관리자 세션(eum_session) 또는 ?key=<토큰> 접근 (불일치 시 404)
// ────────────────────────────────────────────────────────────
const adminStats = new Hono<HonoEnv>()

adminStats.get('/admin/stats', async (c) => {
  let authed = false
  const session = getCookie(c, 'eum_session')
  const secret = getAuthSecret(c.env as any)
  if (session && secret) {
...
<title>검색·방문 통계 — 이음치과 관리자</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Noto Sans KR',sans-serif;background:#0a1128;color:#f8f9fc;min-height:100vh}
.stats-top{background:#121d45;border-bottom:1px solid rgba(36,58,138,.45);padding:14px 22px;display:flex;align-items:center;justify-content:space-between;gap:14px}
.stats-top strong{font-size:1rem;letter-spacing:.02em}
.stats-top strong em{color:#6e9fd4;font-style:normal;margin-right:8px}
.stats-top a{color:rgba(248,249,252,.7);text-decoration:none;font-size:.84rem;font-weight:700;border:1px solid rgba(36,58,138,.6);border-radius:8px;padding:7px 13px}
.stats-top a:hover{color:#f8f9fc;background:#182456}
</style>
</head>
<body>
<header class="stats-top"><strong><em>이음</em>검색·방문 통계</strong><a href="/admin">관리자 홈</a></header>
