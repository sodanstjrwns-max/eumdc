...
/** 홈페이지 가시 FAQ — JSON-LD와 화면 콘텐츠가 반드시 일치해야 함 (Google 정책) */
...
const SEO_KEYWORD_DICT = {
  // 핵심 진료 (원장님 지시: 임플란트, 인비절라인, 라미네이트, 치아교정)
  treatments: [
    '임플란트', '인비절라인', '라미네이트', '치아교정',
...
export function extractSeoKeywords(opts: {
  ...
  // 핵심 진료가 앞으로 오게 정렬
  const priorityKw = ['임플란트', '인비절라인', '라미네이트', '치아교정']
  arr.sort((a, b) => {
...
export function autoGenerateKeywords(opts: {
  ...
  // 핵심 진료 키워드가 모두 누락되면 강제 추가 (메모리 지시)
  const mustHave = ['임플란트', '인비절라인', '라미네이트', '치아교정']
